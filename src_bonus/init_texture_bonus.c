...
/*   Updated: 2024/11/13 16:26:17 by florian          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	fill_colors(char *buffer, int *colors, size_t len)
{
	char	**tmp;
	int		i;
	...
	while (tmp[++i] && tmp[i][0])
	{
		...
		if (colors[i] < 0 || colors[i] > 255)
			return (free_map(tmp, len), \
					ft_perror("Invalid colors value in file\n"));
	}
	return (free_map(tmp, len), 0);
}
